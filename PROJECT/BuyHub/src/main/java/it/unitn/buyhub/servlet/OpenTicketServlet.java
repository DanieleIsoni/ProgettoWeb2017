    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (owner != null && productId != null && !productId.equals("")) {
                ...
                try {
                    Ticket tick = ticketDao.getByOrder(order);
                } catch (Exception ex2) {
                    //MUST CREATE NEW
                    ...
                    Mailer.mail(ph.getValue("noreplyMail"), owner.getEmail(), "Opened ticket", "TESTO CON LINK", ph.getValue("baseUrl"), "Take a look on BuyHub");
                    Mailer.mail(ph.getValue("noreplyMail"), order.getShop().getOwner().getEmail(), "Opened ticket", "TESTO CON LINK", ph.getValue("baseUrl"), "Take a look on BuyHub");
                    Mailer.mailToAdmins(ph.getValue("noreplyMail"), "Opened ticket", "TESTO CON LINK", ph.getValue("baseUrl"), "Take a look on BuyHub", getServletContext());
                    ...
                    not.setDescription("Opened new ticket");
        ...
        response.sendRedirect(response.encodeRedirectURL(contextPath + "restricted/ticket.jsp"));
    }
